function direc = eigdecompose(line,t,figname,dis)
...
figure('Name',['eigen no: ',num2str(figname)]);
...
for i = 1:length(t)
    ...
    for j = 1:size(line,2)
        if ~isempty(line{i,j})
            ...
            plot(x1,y1,'o','Color','yellow','LineWidth',1); %绘制出eig分解后由线段质心推算出的线段端点
            plot(x2,y2,'o','Color','yellow','LineWidth',1); %绘制出eig分解后由线段质心推算出的线段端点
            plot([x1,x2],[y1,y2],'LineWidth',1,'Color','red');
%             %绘制出由角点定的直线，进行比较
...
%             plot(x_cor,y_cor,'LineWidth',1,'Color','blue');   
    ...
    for j=1:length(xy1)
        ...
        plot(points(:,1),points(:,2),'LineWidth',1,'Color','yellow');
